      ...
      rl.question('Type an option: ', (option) => {
        switch (option) {
          ...
          case "4":
            ...
            db.collection('companies').find({},{name: 1, number_of_employees:1, _id: 0 }).sort({"number_of_employees":-1}).limit(9).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
            ...
            db.collection('companies').find({$and:[{name:"Facebook"},{"relationships.is_past":false}]},{"relationships.person.first_name":1}).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
            ...
            db.collection('companies').find({$and:[{"relationships.person.last_name":"Ebersman"},{"relationships.person.first_name":"David"},{"relationships.is_past":true}]},{name:1}).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
            ...
            case "13":
              ...
              db.collection('companies').find({tag_list:/social-networking/},{name:1}).toArray((error, result) => {
                if (error) {
                  console.log(error);
                  rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                } else {
              ...
              db.collection('companies').find({$and:[{tag_list:/social-network/},{"founded_year":{$gte:2002}},{"founded_year":{$lte:2016}},{"offices.city":"New York"}]},{name:1}).count((error, result) => {
                if (error) {
                  console.log(error);
                  rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                } else {
